...
//			intake.raise();
		...
		// ENGAGING TURRET
		if (state == OperatorState.IDLE && controller.getButton(GamepadButton.LT) /* && intake.getState() == IntakeState.DOWN */) {
		...
		if (state == OperatorState.TURRET_ENGAGED && shooter.getState() == ShooterState.IDLE && controller.getButton(GamepadButton.RT)) {
			shooter.arm();
			// DriverStation.reportError("\n ARMING", false);
		} else if (state == OperatorState.TURRET_ENGAGED && (shooter.getState() == ShooterState.ARMING || shooter.getState() == ShooterState.ARMED ) && (!controller.getButton(GamepadButton.RT))) {
			// DriverStation.reportError("\n DISARMING", false);
			shooter.idle();
		}
