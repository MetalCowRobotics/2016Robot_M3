...
public class OperatorController {
	...
	public void drive(CowInputGamepad controller) {
		turret.prestep();
		...
		// Raise Arm
		if (controller.getDpadY() == 1 && state == OperatorState.IDLE) {
		...
		// Lower Arm
		if (controller.getDpadY() == -1 && state == OperatorState.INTAKE_RAISED) {
			// DriverStation.reportError("\n LOWERING", false);
			idleAll();
			state = OperatorState.IDLE;
		}
	...
	public void manualTurretDrive(CowInputGamepad controller, double speedMod) {
	...
	public boolean isAllIdle() {
		...
		shooter.getState() == ShooterState.IDLE && turret.getState() == TurretState.IDLE;
	}
